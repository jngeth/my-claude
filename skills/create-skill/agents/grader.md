...
Beyond grading the run, judge the evals themselves. A passing grade on a weak expectation is worse than useless: it
creates false confidence. An expectation is _discriminating_ when it passes only if the skill genuinely succeeds and
fails when it does not. Surface a suggestion only when there is a clear gap, not for every expectation. Worth raising:

- An expectation that passed but would also pass for a clearly wrong output (e.g. checks a filename, not the content).
- An important outcome you observed, good or bad, that no expectation covers.
- An expectation that cannot be verified from the available outputs.
...
- `{outputs_dir}/user_notes.md`: uncertainties or issues executor flagged. Reveals problems even when expectations pass.
...
```json
{
  ...
  "eval_feedback": {
    "suggestions": [
      ...
      {
        "reason": "No expectation checks whether extracted phone numbers match the input: wrong numbers went uncaught"
      }
    ],
...
- `eval_feedback`: the eval critique, only when warranted. Each suggestion carries a `reason` and an optional
  `expectation` it relates to. `overall` can be "No suggestions, evals look solid" when nothing needs flagging.
